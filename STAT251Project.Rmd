...
$$a^*_\text{USA}=a+y=1.5+873,000=873,001.5$$
...
plot(x, dbeta(x, astar.CHN, bstar.CHN), type = "l",
     main = substitute(paste(pi, "(", theta, "|y)", " ~ Beta(a = ",
                             a, ", b = ", b, ")",  sep = ""),
...
According to our data character, we choose a Bayesian approach to analyize our parameter of interests, which are whether cardiovascular disease is a more prevalent cause of death in the United States than China and elsewhere. 

The model we chose is binomial beta distribution, and our prior belief for people passed away due to cardiovascular disease is follow a Beta distribution with shape = 1.5 and rate = 10. We have also found the conjugate posterior distribution for death cause is cardiovascular disease in the USA, China and all othercountry. 

We did posterior distribution comparison between the USA and China, the USA and all other countries. According to the large number of draws from the posterior difference, we conclude that our null hypothesis is wrong. In the USA and China comparsion, there is a 95% probability that the proportion of deaths in the United States due to cardiovascular disease is between -0.131 and -0.130, which is lower than the proportion of deaths due to cardiovascular disease in China. Similar in comparision between the USA and all other countries, there is a 95% probability that the proportion of deaths in the United States due to cardiovascular disease is between -0.0283 and -0.0272, which is lower than the proportion of deaths due to cardiovascular disease in other countries.

Although we conclude that the proportion of deaths in the United States due to cardiovascular disease is lower than same cause in China and all other countries as a whole, we do not know the reason behind it, and it might be hard to draw a conclusion for why the USA is lower at cardio-disease death, there are tremendous factor contribute to the facts.
